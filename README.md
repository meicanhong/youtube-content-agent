...
- `storyboard.jpg`：默认主成品，固定为 1080x1440（3:4）。顶部约三分之一是大人物画面，下面按时间顺序连续铺满超宽分镜条，不添加白色间隔；字幕居中显示为白字黑描边，并叠加轻量半透明黑色遮罩。整张图使用统一字号且每条只显示一行，长句会拆成相邻的连续字幕条，行尾标点会自动移除；若内容过多，会在限定范围内统一缩小全部字幕，而不是让单条字号不一致。
...
每个选题优先只下载所需连续片段（前后各留 3 秒），避免下载数小时完整视频。若 YouTube 的远程媒体地址拒绝 FFmpeg seek，系统会明确记录 fallback：用 yt-dlp 原生方式下载一次 480p 以内源视频到 `work/` 缓存，随后在本地精确裁切；同一视频的多个选题复用该缓存。每个 Slide 在目标时间附近尝试 `-0.35s / 0s / +0.35s` 三帧，以曝光、对比度和清晰度做轻量评分，选出较自然的一帧。系统同时输出两种视觉：`images/` 保留独立 4:5 图片用于追溯或轮播，`storyboard.jpg` 则把全部分镜合成为一张固定 1080x1440 的连续故事长图，作为默认发布成品。字幕统一字号、逐条单行显示；若固定字号下内容放不进画布，渲染器会统一降低整张图的字号并重新排版，最低字号仍无法容纳时才会明确报错。
