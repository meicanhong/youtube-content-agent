...
YouTube URL
  ...
  -> 按选题下载连续视频片段
  -> 每个时间戳附近三帧质量择优
  -> 1080x1350 独立分镜 + 1080x1440 连续故事长图
...
每段分别生成一张或多张连续 `storyboard`；没有合格内容时输出 0 个主题，不会为了凑数生成弱内容。
...
└── 01-topic-slug/
    ...
    ├── metadata.json
    ├── storyboard.jpg               # 单页主题
    ├── storyboard-01.jpg            # 多页主题第 1 页
    ├── storyboard-02.jpg            # 多页主题第 2 页
    └── images/
        ├── 01.jpg
        ├── 02.jpg
        └── ...
...
- `content.json`：中文选题、Hook、Caption、6-14 个 Slide；每个 Slide 同时保存时间戳、附近英文原文、中文和图片路径。
- `metadata.json`：字幕来源、Editorial provider、分镜数、按顺序排列的 `storyboard_images` 和是否完整。
- `storyboard.jpg` / `storyboard-*.jpg`：默认主成品，固定为 1080x1440（3:4）。顶部约三分之一是大人物画面，下面按时间顺序连续铺满超宽分镜条，不添加白色间隔；字幕居中显示为白字黑描边，并叠加轻量半透明黑色遮罩。整组图片使用统一字号且每条只显示一行，长句会拆成相邻字幕条，首尾标点都会自动移除。清洗后超过 10 条时自动生成 `storyboard-01.jpg`、`storyboard-02.jpg` 等连续页，不会为了塞进一张图而截断论述。
...
- 选题必须对应 20-240 秒连续片段。
- 每个选题 6-14 个 Slide；内容超过一页时继续输出有序长图，直到讲完整该段内容。上限控制在 14，避免最多 6 个主题同时返回时超过模型 JSON 输出容量。
...
每个选题优先只下载所需连续片段（前后各留 3 秒），避免下载数小时完整视频。若 YouTube 的远程媒体地址拒绝 FFmpeg seek，系统会明确记录 fallback：用 yt-dlp 原生方式下载一次 480p 以内源视频到 `work/` 缓存，随后在本地精确裁切；同一视频的多个选题复用该缓存。每个 Slide 在目标时间附近尝试 `-0.35s / 0s / +0.35s` 三帧，以曝光、对比度和清晰度做轻量评分，选出较自然的一帧。系统同时输出两种视觉：`images/` 保留独立 4:5 图片用于追溯或轮播，`storyboard.jpg` 或有序的 `storyboard-*.jpg` 则把分镜合成为固定 1080x1440 的连续故事长图。每页最多 10 个字幕条；不足两条的末页会自动与前一页重新平衡。字幕统一字号、逐条单行显示，并清理首尾标点。
